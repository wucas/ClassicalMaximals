 ...
 for cl in classes do
   if cl eq 1 then
     ...
     if type eq "L" then
        ...
        Exclude(~divs,1);
        for t in divs do
          if (t eq d and d gt 2 and q le 4) or (t eq d div 2 and q le 2) then
     ...
     (d eq 4 and t eq 2 and q eq 3) or (d eq 4 and t eq 4 and q eq 5) then
       ...
       if IsEven(d) then
         if sign eq 1 and d mod 4 eq 0 and not (d eq 8 and q le 3) then
...
*/
       ...
       if type eq "L" then
         if novelties then continue; end if;
         if e eq 1 and q mod 24 in {1,7,13,19} then
           //Sp63
         ...
         if e eq 1 and q mod 4 eq 3 then
           ...
           if all then nconj :=  GCD(q+1,14);
              ...
              dual := sub< Generic(S[1]) |
                              [Transpose(S[1].i)^-1 : i in [1..Ngens(S[1])]] >;
              dual := normaliser select dual^TransformForm(dual:Scalars:=true)
                      else dual^TransformForm(dual);
              asmax cat:= GConjugates(dual,C,nconj);
           else Append(~asmax, S[1]);
           end if;
         end if;
         ...
         if sign eq 1 then
           if novelties then
             if e eq 1 and q mod 156 in {1,25,43,49,55,61,79,
                      103,121,127,133,139} then
                //L2_13a
...
*/
           ...
           if e eq 1 and q mod 156 in {5,11,41,47,59,71,83,
                    89,119,125,137,149} then
              //L2_13a
             _LRS := Read(c9lib cat "l213d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
           ...
           if (e eq 1 and q mod 156 in {11,47,59,71,83,119,5,41,89,125,137,149})
                                                                or q eq 2 then
              //L2_13b
             _LRS := Read(c9lib cat "l213d14b");
             _LR := eval _LRS;
...
*/
           ...
           if novelties then
             if e eq 1 and q mod 156 in {17,23,29,35,53,77,
                                 95,101,107,113,131,155} then
                //L2_13a
               _LRS := Read(c9lib cat "l213d14a");
               _LR := eval _LRS;
               S := ReduceOverFiniteField(_LR,qq :
           ...
           if (e eq 1 and p mod 60 in {11,17,29,41,53,59,23,47} and p ne 7) or
             ...
             S := ReduceOverFiniteField(_LR,qq :
                 special:=special, general:=general, normaliser:=normaliser);
             if all and q eq 2 then asmax cat:= [S[1], S[1]^CS];
           ...
           if e eq 1 and q mod 156 in {7,19,31,37,67,73,85,
                               97,109,115,145,151} then
              //L2_13a
             _LRS := Read(c9lib cat "l213d14a");
             _LR := eval _LRS;
             S := ReduceOverFiniteField(_LR,qq :
